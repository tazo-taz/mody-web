...
export default function Stage2({ phone, code, onVeriticationCodeChange, goBack, sendAuthSms }: Stage2Props) {
    ...
    return (
        <>
            ...
            <VerificationInput value={code} setValue={onVeriticationCodeChange} count={codeLength} />

            <div className='mt-5'>
                <Button disabled={!!seconds} onClick={resend} size='sm' className='rounded-full font-medium' variant='secondary'>{getLanguageItem("Resend_code")} ({moment.utc(seconds * 1000).format('mm:ss')})</Button>
            </div>
